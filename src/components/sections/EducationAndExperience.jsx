...
import { SiHackerrank, SiLinkedin, SiEducative } from "react-icons/si";
              ...
              {user_info.certificates.map((cert, index) => {
                return (
                  <div className="hs-carousel-slide relative" key={index}>
                    {cert.icon === "linkedinlearning" ? (
                      <SiLinkedin className="text-blue-500 absolute right-5 top-3 text-3xl" />
                    ) : cert.icon === "study" ? (
                      <SiEducative className="text-black-500 absolute right-5 top-3 text-3xl" />
                    ) : (
